...
3. 閉じた後、各辺の **○ マーク**をクリックして **直線 (L) / 円弧 (A)** を切替
4. 円弧の場合は「**膨らみ量**」スライダーで形状を調整（負 = 左膨らみ、正 = 右膨らみ）
5. 「**✔ ピースを追加**」で確定 → 次のピースに進む
6. 全ピースが揃ったら「**キャンセル**」で手描きモードを終了

#### スナップグリッド

スナップ設定（1/4, 1/6, 1/8, 1/10, 1/12）で頂点位置を格子点に自動吸着できます。

#### よく使うパターンの例

| パターン | 構成 |
|---|---|
| Drunkard's Path | 2 頂点の扇形 + 1 辺を円弧（凸） |
| Orange Peel | 4 頂点の正方形 + 対向 2 辺を円弧 |
| Fan blade | 扇形（1 辺を大きな円弧に） |
