...
- [ ] `buildPdfHtml` の SVG 出力で `width="${sizeMm}mm" height="${sizeMm}mm"` を使用
- [ ] viewBox は `0 0 sizeMm sizeMm`(または 0..1 のまま `transform: scale(sizeMm)`)
- [ ] 用紙サイズ A4(210×297mm)印刷可能領域を超える sizeMm の場合、警告ダイアログを出して縮小印刷の許可を取る(あるいは中止)
- [ ] 既存の `scaleNote`(印刷スケール 100% を指定する旨)は維持
- [ ] iOS / Android どちらの印刷ダイアログでも実寸が崩れないこと
