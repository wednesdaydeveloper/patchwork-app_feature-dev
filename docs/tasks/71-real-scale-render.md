...
- [ ] `PieceImage` のシグネチャに `sizeMm` と `pxPerMm` を追加
- [ ] `scale = 1.0` のとき、画像が「画像横幅 mm / sizeMm」の正規化幅で表示される(実寸)
- [ ] `preserveAspectRatio` をデフォルト("xMidYMid meet" など)に変更し、画像の縦横比を維持
- [ ] 画像が piece bbox より小さい場合の扱いを仕様化(空白可、もしくは「画像が小さすぎる」案内)
- [ ] 未キャリブレーション(`pxPerMm = null`)布地は旧来の cover ロジックでフォールバック描画
- [ ] エディタプレビュー / オフスクリーンエクスポート両方で機能
- [ ] 既存テストがパス(必要なら期待値を更新)
