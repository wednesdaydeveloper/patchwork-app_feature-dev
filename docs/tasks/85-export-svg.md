...
- [ ] エクスポート画面に「SVG として保存」ボタンを追加 (PNG/JPEG の隣)
- [ ] SVG は **単一ファイル完結** (画像は base64 data URI で埋め込み)
- [ ] viewBox `0 0 1 1`、width/height は `${sizeMm}mm` で実寸再現
- [ ] ピース形状を ClipPath、布地は `<image>` で配置 (PDF 出力と同じ計算式)
- [ ] 出力後にファイル保存ダイアログ (`expo-sharing` の `shareAsync` 経由でファイル保存・他アプリ送信を選択可能) を表示
- [ ] 失敗時はエラートーストで通知し、再試行可能
- [ ] 既存テストグリーン / 型チェック通過
