...
CLAUDE.md「用語定義」のパターン制約のうち、現在は**座標範囲**のみを機械的に検証する。
重なり検査・面積合計検査は円弧パスの近似誤差により誤検知が多いため削除済み。

- 実行タイミング: 開発ビルド時 / テストでのみ実行（本番ビルドではスキップ）
- 使用ライブラリ: `svg-path-properties`
- 検証ステップ:
  1. 各 `Polygon.path` を SVG パーサで近似ポリラインに変換（曲線は十分な解像度でサンプリング）
  2. 全頂点・サンプル点が `[0, 1]` の範囲内であることを確認 → NG なら `error.designOutOfBounds`
- 失敗時は警告ログを出力し、該当デザインも含めてロードを継続する
