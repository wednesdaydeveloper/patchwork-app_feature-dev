...
| ジェスチャー     | react-native-gesture-handler                    | パン操作（位置）・2 本指回転（rotation）                       |
...
| 共有 / 保存先選択 | expo-sharing                                    | 共有シートで保存先（写真 / ファイル / 他アプリ等）を選択       |
...
- パターンのサムネイルをタップして選択 → サイズ指定画面へ遷移
- サイズ指定画面: パッチワークの一辺の物理サイズを mm 単位で指定(範囲 50〜280mm、初期値 150mm)
- サイズ確定後、パッチワーク作成画面へ遷移する
- `Work.sizeMm` として保存され、後からエディタヘッダーで編集可能
...
| 回転     | 画像の回転角度を調整（2 本指回転ジェスチャ）                |

- 選択されたピースを調整する場合は「調整」ボタンを押す。
- 「調整」ボタンをタップすると、調整しやすいようにピースを大きく表示する。
- 調整モード中は画面下部の布地選択パネル（`FabricPicker`）を非表示にし、調整操作に集中できるようにする。
- 調整はリアルタイムにプレビューへ反映される
- 調整値（位置・回転）はピースごとに独立して保存される
...
- 完成したパッチワークを画像として保存できる
- 保存方式: 共有シート（`Sharing.shareAsync`）を起動し、ユーザーが保存先を選択する
  - 写真ライブラリ / ファイル（iCloud Drive・Files・SAF 経由のフォルダ）/ クラウドストレージ / 他アプリ等、OS が提供する選択肢から選べる
  - OS が「フォルダ選択ダイアログ」を直接提供しないため、共有シート経由を採用する
...
| 印刷ダイアログのキャンセル | silent で扱う（ダイアログ・トースト・ログ出力なし）。ユーザー意図のキャンセルは失敗ではないため。具体的には `expo-print` の `ERR_PRINT_INCOMPLETE` を判別する |
...
- 設定画面で「日本語」「English」のいずれかを選択可能（初回起動時は端末ロケールから自動判定）
...
├── name: String
├── designId: String
├── sizeMm: Number          // パッチワーク一辺の物理サイズ（50〜280mm、デフォルト 150）
├── createdAt: DateTime
├── updatedAt: DateTime
...
└── rotation: Float      // 画像中心まわりの回転（ラジアン、デフォルト 0）
```
...
- 仕様変更を依頼された際は、いきなり修正せず **`docs/tasks/NN-xxx.md` を作成し、必要なら CLAUDE.md を更新**してから実装に進む
- **不具合修正(bug fix)のコミットはユーザーの実機確認後に行う**。修正コードを書いたらコミットせずに動作確認を依頼し、「直った」と確認できてから `git commit` する。確認なしのまま履歴に積むと、効果のない/逆効果の変更が残り後追い・revert の負担が増える。機能追加・ドキュメント・タスク化など bug fix 以外は従来通り即コミットしてよい
- **機能追加・不具合修正は専用ブランチで作業し、PR を作成する**。`main` から `feature/<topic>`(機能) または `fix/<topic>`(不具合) のブランチを切り、コミット後に `git push -u origin <branch>` でプッシュし、`gh pr create --base main` で `main` 向けプルリクエストを作成する。マージはユーザーが行う。bug fix は前項のとおり**実機確認後にコミット → push → PR** の順で進める。CLAUDE.md 更新やドキュメントのみの軽微な変更でも、本ワークフローでブランチ＋PR を作成する
- git のコメントを含め、日本語を使う。
- 上記ワークフロー外での `git push` は行わない（`main` への直接プッシュ・他ブランチへの無断プッシュ禁止）。
